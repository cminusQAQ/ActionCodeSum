...
class CoConLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 max_relative_positions=0,
                 use_neg_dist=True):
        super(CoConLayer, self).__init__()
    ...
    def forward(self, key, value, query, mask):
        ...
        Returns:
            ...
            * outputs `[batch_size x src_len x model_dim]`
        """
        context, attn_per_head, _ = self.attention(key, value, query,
                                                   mask=mask, attn_type="self")
        out = self.layer_norm(self.dropout(context) + query)
        return self.feed_forward(out), attn_per_head

class CoCon(EncoderBase):
    def __init__(self,
                 num_layers,
                 d_model=512,
                 heads=8,
    ...
    def forward(self, src, act, ag, lengths=None):
        """
        Args:
            src (`FloatTensor`): `[batch_size x src_len x model_dim]`
            lengths (`LongTensor`): length of each sequence `[batch]`
        ...
        out = src
        key = torch.cat((src, act, ag), dim=1)
        mask = None if lengths is None else \
            ~sequence_mask(lengths, out.shape[1]).unsqueeze(1)
        # Run the forward pass of every layer of the tranformer.
        representations = []
        attention_scores = []
        for i in range(self.num_layers):
            out, attn_per_head = self.layer[i](key, key.clone(), out, None)
            representations.append(out)
            attention_scores.append(attn_per_head)
